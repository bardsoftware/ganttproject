...
BEGIN
    IF NEW.name IS DISTINCT FROM OLD.name OR NEW.num IS DISTINCT FROM OLD.num THEN
    ...
    IF NEW.start_date IS DISTINCT FROM OLD.start_date OR NEW.duration IS DISTINCT FROM OLD.duration OR NEW.earliest_start_date IS DISTINCT FROM OLD.earliest_start_date THEN
