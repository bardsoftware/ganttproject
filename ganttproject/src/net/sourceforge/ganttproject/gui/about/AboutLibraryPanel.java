...
public class AboutLibraryPanel extends GeneralOptionPanel {
  ...
  public AboutLibraryPanel() {
    super(GanttLanguage.getInstance().getText("library"), GanttLanguage.getInstance().getText("settingsLibrary"));

    JTable jTableLib = new JTable();
    LibraryFieldTableModel modellib = new LibraryFieldTableModel();

    jTableLib.setModel(modellib);
    modellib.addField(new LibraryInfo("ganttproject-" + GanttProject.version + ".jar", "ganttproject.biz",
        "Project Manager."));
    ...
    modellib.addField(new LibraryInfo("jdnc-modifBen.jar", "https://jdnc.dev.java.net/", "Swing components"));
    modellib.addField(new LibraryInfo("mpxj_0.0.25_CVS.jar", "http://mpxj.sourceforge.net/",
