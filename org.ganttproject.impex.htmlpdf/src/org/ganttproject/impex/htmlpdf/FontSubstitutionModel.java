...
public class FontSubstitutionModel {
    ...
    private final ArrayList myIndexedSubstitutions;
    ...
    public FontSubstitutionModel(TTFontCache fontCache, ITextStylesheet stylesheet, Preferences prefs) {
        ...
        for (Iterator families = stylesheet.getFontFamilies().iterator();
             families.hasNext();) {
            String nextFamily = (String) families.next();
        ...
        myIndexedSubstitutions = new ArrayList(mySubstitutions.values());
    }

    private void addSubstitutions(List substitutions) {
        for (int i=0; i<substitutions.size(); i++) {
            FontSubstitution nextSubstitution = (FontSubstitution) substitutions.get(i);
    ...
    public FontSubstitution getSubstitution(String originalFamily) {
        return (FontSubstitution) mySubstitutions.get(originalFamily);
    }
    public FontSubstitution getSubstitution(int index) {
        return (FontSubstitution) myIndexedSubstitutions.get(index);
    }
    ...
    public boolean hasUnresolvedFonts() {
        for (Iterator substitutions = mySubstitutions.values().iterator();
             substitutions.hasNext();) {
            FontSubstitution fs = (FontSubstitution) substitutions.next();
            if (!fs.isResolved()) {
                return true;
            }
        }
